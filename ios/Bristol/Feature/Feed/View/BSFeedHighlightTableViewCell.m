...
@interface BSFeedHighlightTableViewCell ()<BSPlayerDelegate>
{
    ...
    CGRect _originFrame;
	BOOL _isPlayingWhenResignActive, _isApplicationInActive;
	BOOL _isMute, _isAutoPlayVideo, _isLoopingVideo, _isTapToFullScreen, _isStreaming;
}
	...
	if (_highlight != highlight) {
		...
		ZPVoidBlock imageDidDownloadBlock = ^ {
			...
			dispatch_async(dispatch_get_global_queue(0, 0), ^{
				[_videoCoverImageView sd_setImageWithURL:urlCover placeholderImage:[BSUIGlobal placeholderImage] completed:nil];
			});
		};
...
- (void)checkIfCanPlayVideo {
	if (!_isApplicationInActive && _isAutoPlayVideo && [self isVisibleToPlayVideo]) {
		[self playVideo];
    } else {
        [self pauseVideo];
    }
...
- (void)playerPlayingStateChanged:(BSPlayer *)player {
	ZPLogDebug(@"playing:%@", player.isPlaying ? @"Y" : @"N");
	if (_isStreaming) {
		[self _setProgressViewAnimated:!player.isPlaying];
	} else {
		[self _setProgressViewAnimated:NO];
	}
}
