...
#import "SDAVAssetExportSession+Progress.h"
...
@interface BSCaptureViewController () <UITextFieldDelegate, UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, BSVideoTrimmerViewDelegate, BSPlayerDelegate, CTAssetsPickerControllerDelegate>
{
...
@property (weak, nonatomic) IBOutlet BSCommentTextView *txtDescription;
@property (weak, nonatomic) IBOutlet UIView *maxHeightView;
...
- (void)viewDidLoad {
    ...
    [_btnAvatar configWithUser:[BSDataManager sharedInstance].currentUser];
...
//	[self _testWithDefaultVideo];
    ...
    _txtDescription.maxHeight = _maxHeightView.bounds.size.height;
}
...
- (void)_exportVideo {
	...
	AVAssetExportSession *exportSession = [composition makeExportableWithPresetName:AVAssetExportPreset640x480];
	_exportSession = [SDAVAssetExportSession exportSessionWithAsset:exportSession.asset];
	_exportSession.outputURL = [self _getVideoProcessedFileFullPath];
	_exportSession.outputFileType = AVFileTypeMPEG4;
	_exportSession.shouldOptimizeForNetworkUse = YES;
	_exportSession.audioMix = exportSession.audioMix;
	_exportSession.videoComposition = exportSession.videoComposition;
	...
	AVVideoCompressionPropertiesKey: @
		{
		AVVideoAverageBitRateKey: @933120,
...
- (void)assetsPickerController:(CTAssetsPickerController *)picker didFinishPickingAssets:(NSArray *)assets {
	if (assets.count) {
		...
		AVAssetExportSession *exportSessionWeak = exportSession;
		...
		[exportSession exportAsynchronouslyWithProgress:^(float value) {
			...
			dispatch_main_sync_safe(^ {
				...
				if (exportSessionWeak.status == AVAssetExportSessionStatusCompleted) {
					_videoUrl = exportSessionWeak.outputURL;
					[self _goToHighlightDetectView];
					[self _detectHighlight];
					
					[picker.presentingViewController dismissViewControllerAnimated:YES completion:nil];
				} else {
					[BSUIGlobal showError:exportSessionWeak.error];
					ZPLogDebug(@"Export video error: %@", exportSessionWeak.error);
				}
			});
		}];
	} else {
